   ...
   <section className='bg-stone-50 p-6 text-emerald-950' id='services'>
    <div className='container mx-auto'>
      ...
      <div className='flex items-center justify-center space-x-4'>
        {SERVICES.map((tab) => (
          <button key={tab.id} onClick={() => setActiveTab(tab)} className={`px-4 py-2 font-semibold tracking-tighter transition-colors lg:text-2xl ${activeTab.id === tab.id ? "border-b-2 border-orange-500 text-orange-500" : "text-emerald-950"}`} >
            {tab.title}
          </button>
        ))}
      </div>
      ...
      key={activeTab.id} className='mt-8 flex flex-col items-center lg:flex-row'>
        ...
        <div className='p-4 lg:w-1/2'>
        <img src={activeTab.imgSrc} alt="" className='h-auto w-full rounded-lg ' />
        </div>
      </motion.div>
    </div> 
   </section>
